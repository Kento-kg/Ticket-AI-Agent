...
def classify_ticket(text: str) -> dict:
    ...
    (derived from the category-team mapping)."""
...
@tool
def search_similar_tickets(text: str, category: str | None = None, k: int = 3) -> list[dict]:
    """Retrieve the top-k most similar tickets using semantic search.
    Filters by category via ChromaDB's native metadata filter when provided.
    Returns text, category, urgency, team, and similarity score for each."""
    return search(text, k=k, category=category)
